(function (root, factory) {
  ...
  // Intentionally unregistered PNP ID: GNOME then displays the EDID product
  // name instead of substituting an unrelated manufacturer database entry.
  const DEFAULT_VENDOR = "ZVD";
  const DEFAULT_NAME = "Virtual Disp";
  ...
  function generateEdid({ defaultMode, modes, audio, hdr, deepColor, vrr, dsc, listedModesOnly }) {
    ...
    const needsHfVsdb = maxTmdsMhz > 340 || Boolean(vrrRange) || requiresFrl || dscEnabled;
    const hdmiVersion =
